(() => {
  ...
  const sources = {
    ...
    ihsan: { label: "Ihsan et al. (2013), physiological effects of cooling", url: "https://pubmed.ncbi.nlm.nih.gov/24004719/" },
    ...
    raynaud: { label: "Herrick et al. (2017), Raynaud phenomenon guidance", url: "https://pubmed.ncbi.nlm.nih.gov/28895508/" },
  ...
  const articles = [
    {
      ...
      refs: ["algafly", "ihsan", "cochrane", "allan", "nerveCases", "fda", "urticaria", "raynaud"]
    },
    {
      ...
      refs: ["temperatures", "perfusion", "algafly", "ihsan"]
    },
    {
    ...
    },
    {
      ...
      refs: ["algafly", "ihsan", "postop", "cochrane"]
    },
    {
  ...
  const related = articles.filter((entry) => entry.slug !== article.slug).slice(Math.max(0, Number(article.order) - 2), Math.max(0, Number(article.order) - 2) + 2);
